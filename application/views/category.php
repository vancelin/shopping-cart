   ...
   $(function(){
        $("#item_content").infinitescroll({
             ...
             path: function(index) {
                ...
                return "<?php echo base_url('category/index/') . '/' . $category_id . '/';?>" + (index-1);
             }
        });
                ...
                <div id="scroll">
                    <div id="item_content" class="wrap white-space-clear">
                        <?php foreach($products as $key => $product):?>
                            <div class="item">
                                <table class="filled-table tool-bar">
                                    <tbody>
                                        <tr>
                                            ...
                                            <td><button productid="<?php echo $product['id'];?>" class="product-tool-button quickshop">加入</button></td>
                                            <?php else:?>
                                            <td><button class="product-tool-button">售完</button></td>
                                            <?php endif;?>
                                            <td><button productid="<?php echo $product['id'];?>" class="follow product-tool-button">追蹤</button></td>
                                        </tr>
                                    </tbody>
                                </table>
                                <a href="<?php echo base_url('home/product/'.$product['id']);?>" target="_blank" alt="See More ...">
                                    ...
                                    <div class="item-content">
                                        ...
                                        <?php if($product['id'] == $imgs[$key]['product_id']):?>
                                            <img src="<?php echo base_url('public/product_imgs/'.$imgs[$key]['img_name']);?>" title="<?php echo $product['product_name'];?>" />
                                        <?php endif;?>
                                        </div>
                                        <?php if($product['bargain_price']!='0'):?>
                                            <?php if(date("Y-m-d",strtotime($product['bargain_from'])) <= date("Y-m-d",strtotime(date("Y-m-d"))) && date("Y-m-d",strtotime($product['bargain_to'])) >= date("Y-m-d",strtotime(date("Y-m-d"))))://判斷折價是否過期?>
                            ...
                            <?php endforeach;?>
                            <a id="next" href="<?php echo base_url('category/index') . '/' . ($this->uri->segment(3, 0) + 1);?>">next</a>
                    </div>
                </div>
