...
/*
 * 2013/01/28 因使用 uploadify 所以增加此檔案 參考 http://ellislab.com/forums/viewthread/221152/
 * 
 */
class MY_Session extends CI_Session
{
 public function __construct() {
  ...
  if ($this->userdata('user_agent') == "Shockwave Flash") {
   ...
   if ($this->userdata('manage_logged_in')) {
   ...
   //not yet logged in
    ...
    if ($sessdata) {
     ...
     if ($parent_session['manage_logged_in']) {
      ...
      $this->set_userdata('manage_logged_in', $parent_session['manage_logged_in']);
      $this->set_userdata('id', $parent_session['id']);
